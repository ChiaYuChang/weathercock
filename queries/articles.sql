-- name: InsertTestUserArticle :one
...
VALUES ($1, $2, $3, $4, $5)
RETURNING id;
-- name: InsertUsersChunksBatch :batchone
INSERT INTO users.chunks (
        article_id,
        "start",
        offset_left,
...
WHERE a.id = $1
ORDER BY c."start";
-- name: GetUsersArticleByID :one
SELECT *
FROM users.articles
WHERE id = $1;
-- name: GetUsersArticleByTaskID :one
SELECT *
FROM users.articles
WHERE task_id = $1;
-- name: GetUsersArticleByMD5 :one
SELECT *
FROM users.articles
WHERE md5 = $1;
...
SELECT *
FROM articles
WHERE published_at BETWEEN sqlc.arg('start') AND sqlc.arg('end')
ORDER BY published_at DESC
LIMIT sqlc.arg('limit')::integer;
-- name: GetArticlesInPastKDays :many
SELECT *
FROM articles
WHERE published_at >= NOW() - INTERVAL '1 day' * sqlc.arg(k)::integer
ORDER BY published_at DESC
LIMIT sqlc.arg('limit')::integer;
...
VALUES ($1, $2, $3, $4, $5)
RETURNING id;
-- name: InsertChunksBatch :batchone
INSERT INTO users.chunks (
        article_id,
        "start",
        offset_left,
