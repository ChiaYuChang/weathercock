...
VALUES ($1, $2, $3, @vector::vector)
RETURNING id;
-- name: InsertEmbeddingBatch :batchone
INSERT INTO embeddings (
        article_id,
        chunk_id,
        model_id,
...
VALUES ($1, $2, $3, @vector::vector) ON CONFLICT DO NOTHING
RETURNING id;
...
WHERE e.article_id = ANY(@article_ids::integer [])
...
VALUES ($1, $2, $3, @vector::vector)
RETURNING id;
-- name: InsertUsersEmbeddingBatch :batchone
INSERT INTO users.embeddings (
        article_id,
        chunk_id,
        model_id,
...
VALUES ($1, $2, $3, @vector::vector) ON CONFLICT DO NOTHING
RETURNING id;
-- name: GetAverageUsersEmbeddingByArticleIDs :one
SELECT e.article_id,
    e.model_id,
    AVG(e.vector)::vector AS avg_vector
...
WHERE e.article_id = ANY(@article_ids::integer [])
    ...
    AND e.vector <> '[]'::vector
GROUP BY e.article_id;
-- name: GetKNNUsersEmbeddingsByL2Distance :many
SELECT article_id,
    (vector <->@query::vector)::float8 AS distance -- <-> is the L2 distance operator in pgvector
...
ORDER BY vector <->@query
LIMIT @k::integer;
-- name: GetKNNUsersEmbeddingsByCosineSimilarity :many
SELECT article_id,
    (vector <=> @query::vector)::float8 AS similarity -- <=> is the cosine distance operator in pgvector
FROM users.embeddings
WHERE model_id = @model_id::integer
...
ORDER BY vector <=> @query
LIMIT @k::integer;
-- name: GetKNNUsersEmbeddingsByInnerProduct :many
SELECT article_id,
    (vector <#>@query::vector)::float8 AS inner_product -- <#> is the inner product operator in pgvector
FROM users.embeddings
WHERE model_id = @model_id::integer
